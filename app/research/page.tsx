...
export default function ResearchPage() {
  const [ideas, setResearch] = useState<IdeaNote[]>([])
  ...
  useEffect(() => {
    fetch('/api/ideas').then(r => r.json()).then(d => {
      ...
      if (!scratchpad) {
        ...
        fetch('/api/ideas', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(pad) })
        setResearch([pad, ...all])
        setActiveId(pad.id)
      } else {
        setResearch(all)
  ...
  const saveIdea = (idea: IdeaNote) => {
    ...
    saveTimeout.current = setTimeout(() => {
      fetch('/api/ideas/' + idea.id, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(idea) })
    }, 800)
  }

  const updateIdea = (id: string, updates: Partial<IdeaNote>) => {
    setResearch(prev => prev.map(i => {
      if (i.id !== id) return i
  ...
  const createNote = async () => {
    ...
    await fetch('/api/ideas', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(note) })
    setResearch(prev => [prev.find(i => i.is_scratchpad)!, note, ...prev.filter(i => !i.is_scratchpad)])
    setActiveId(note.id)
  ...
  return (
    <div style={{ maxWidth: 1000, margin: '0 auto', padding: '0 16px 80px' }}>
      ...
      <div style={{ padding: '28px 0 24px', borderBottom: '1px solid var(--border)', marginBottom: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
        <div>
          <h1 style={{ fontFamily: 'Playfair Display, serif', fontSize: 28, fontWeight: 700 }}>Research</h1>
