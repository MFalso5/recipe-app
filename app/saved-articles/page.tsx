...
export default function FoodForThoughtPage() {
  ...
  useEffect(() => {
    fetch('/api/food-for-thought').then(r => r.json()).then(d => {
  ...
  const saveEntry = async () => {
    ...
    if (editingId) {
      await fetch('/api/food-for-thought/' + editingId, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(entry) })
      setEntries(prev => prev.map(e => e.id === editingId ? entry : e))
    } else {
      await fetch('/api/food-for-thought', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(entry) })
  ...
  const deleteEntry = async (id: string) => {
    ...
    await fetch('/api/food-for-thought/' + id, { method: 'DELETE' })
    ...
    <div style={{ minHeight: '100vh', background: 'var(--cream)' }}>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '28px 20px 80px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 24 }}>
          <div>
            <h1 style={{ fontFamily: 'Playfair Display, serif', fontSize: 28, fontWeight: 700 }}>Saved Articles</h1>
